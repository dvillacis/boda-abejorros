...
======================================================= */
    $('#send_message').click(function(e){
            ...
            var requerimientos = $('#rsvp-extra').val();

            var cena = $('#familyDinner').val();
            var ensayo = $('#CeremonyRehersal').val();
            var boda = $('#theWedding').val();
            var civil = $('#weddingRehearsal').val();
            var despedidaH = $('#bachelorsParty').val();
            var despedidaM = $('#theGirlsParty').val();
            ...
            if(error == false){
                ...
                var request = gapi.client.bodaAbejorrosAPI.bodaabejorros.guardarAsistente(
                    {
                        'nombre':name,
                        'email':email,
                        'mensaje':message,
                        'requerimientos':requerimientos,
                        'eventos':cena + '-' + ensayo + '-' + civil + '-' + despedidaH + '-' + despedidaM
                    }
                );
...
======================================================= */
    ...
    $('#send_guestbook').click(function(e){
        ...
        if(guesterror == false){
            ...
            var request = gapi.client.bodaAbejorrosAPI.bodaabejorros.guardarMensaje(
                {
                    'nombre':guestname,
                    'email':guestemail,
            ...
            request.execute(function(resp) {
                if(resp != null){
                    ...
                    var mensajeRecibido = new MensajeModel(guestname,guestemail,guestmessage);
                    model.mm.addMensaje(mensajeRecibido);
                }
                else {
                    $('#guestbook_fail').fadeIn(500);
                    $('#send_guestbook').removeAttr('disabled').attr('value', 'Enviar Mensaje');
                }
